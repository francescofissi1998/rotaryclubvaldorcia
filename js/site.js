...
(function () {
  ...
  /* Nota diagnostica: se il sistema chiede di ridurre il movimento, il sito
     lo rispetta e spegne parallasse, comparse e conteggi. E' la causa piu'
     frequente di "non vedo le animazioni". Si controlla in Impostazioni ->
     Accessibilita' -> Effetti visivi (Windows) o Accessibilita' -> Schermo
     (macOS). Il messaggio compare nella console del browser (F12). */
  if (window.console && console.info)
    console.info('Rotary Val d’Orcia — movimento ridotto richiesto dal sistema: ' +
      (reduce ? 'SI — ma gli effetti restano attivi per scelta del Club' : 'no'));
  ...
  /* le comparse valgono anche con movimento ridotto: il CSS le riduce
     a una semplice dissolvenza, senza spostamenti */
  if (false) {
    ...
    /* Le comparse non si affidano all'osservatore: a ogni scorrimento (e una
     volta al caricamento) controllo chi e' entrato nello schermo. Se lo
     script fosse bloccato o l'osservatore non scattasse, il contenuto
     resterebbe invisibile: qui non puo' succedere. */
    var daRivelare = rv.slice();
    function rivela() {
      if (!daRivelare.length) return;
      var soglia = (window.innerHeight || 800) * 0.94;
      for (var i = daRivelare.length - 1; i >= 0; i--) {
        var b = daRivelare[i].getBoundingClientRect();
        if (b.top < soglia && b.bottom > -80) {
          daRivelare[i].classList.add('in');
          daRivelare.splice(i, 1);
        }
      }
    }
    window.addEventListener('scroll', rivela, { passive: true });
    window.addEventListener('resize', rivela);
    /* Il primo giro deve avvenire DOPO che il browser ha disegnato lo stato
       iniziale: chiamandolo subito, gli elementi sopra la piega passavano a
       "visibile" nello stesso fotogramma e la transizione non partiva, cioe'
       comparivano gia' fatti. Due fotogrammi di attesa, piu' un ripiego a
       tempo se le animazioni di fotogramma non sono disponibili. */
    var primo = false;
    function primoGiro() { if (primo) return; primo = true; rivela(); }
    if (typeof requestAnimationFrame === 'function')
      requestAnimationFrame(function () { requestAnimationFrame(primoGiro); });
    setTimeout(primoGiro, 140);
    /* Niente rete di sicurezza a tempo: rivelerebbe anche cio' che sta sotto
       la piega mentre si e' ancora fermi in cima, e scorrendo non si vedrebbe
       piu' nulla comparire. Il controllo qui sopra non puo' fallire, perche'
       legge la posizione invece di aspettare un osservatore. */
  }
  ...
  if (counters.length) {
    /* il conteggio non e- movimento: lo lascio anche con movimento ridotto */
    if (false) {
      counters.forEach(function (el) { el.textContent = formatta(el, parseFloat(el.dataset.count)); });
    } else {
      /* stessa logica delle comparse: guardo la posizione invece di
         affidarmi a un osservatore, cosi' il conteggio parte sempre */
      var daContare = counters.slice();
      counters.forEach(function (el) { el.textContent = formatta(el, 0); });
      var avvia = function () {
        if (!daContare.length) return;
        var soglia = (window.innerHeight || 800) * 0.88;
        for (var i = daContare.length - 1; i >= 0; i--) {
          var b = daContare[i].getBoundingClientRect();
          if (b.top < soglia && b.bottom > 0) { conta(daContare[i]); daContare.splice(i, 1); }
        }
      };
      window.addEventListener('scroll', avvia, { passive: true });
      window.addEventListener('resize', avvia);
      avvia();
    }
  }
  ...
  /* la parallasse resta attiva anche con movimento ridotto: scelta
     esplicita del committente */
  ...
  function misura() {
    ...
    for (var i = 0; i < livelli.length; i++) {
      ...
      if (L.bed) {
        ...
        /* la corsa deve stare dentro il margine rimasto dopo lo scostamento,
           altrimenti in un estremo comparirebbe una striscia vuota */
        L.shift = L.over * 0.92;
        ...
        /* --corsa, se dichiarata, impone quanto puo' muoversi: serve alle
           sezioni gia' spostate con --alza, dove non resta margine */
        var corsa = getComputedStyle(L.el).getPropertyValue('--corsa').trim();
        if (corsa !== '') L.shift = parseFloat(corsa) || 0;
        L.bias = 0;
      }
    }
    pronto = true;
  ...
  function disegna() {
    ...
    for (var i = 0; i < livelli.length; i++) {
      ...
      /* dividendo per una frazione di schermo la corsa si compie prima:
         stessa ampiezza, movimento piu- rapido e quindi ben visibile */
      var p = (top + L.h / 2 - vh / 2) / (vh * 0.55);
      if (p > 1) p = 1; else if (p < -1) p = -1;
      L.el.style.transform = 'translate3d(0,' + ((L.bias || 0) - p * L.shift).toFixed(1) + 'px,0)';
    }
  }
  /* se le animazioni di fotogramma non sono disponibili si disegna subito:
     qui si scrive solo transform, nessuna lettura, quindi non costa nulla */
  function onScroll() {
    if (ticking) return;
    ticking = true;
